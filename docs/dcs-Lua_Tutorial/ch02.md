...
In VS Code (or VSCodium if you prefer a libre build), search for a Lua extension that provides:

- Syntax highlighting
- Diagnostics (linting-like feedback)
- Autocomplete / language server features

Look for “Lua Language Server” support. Different extensions exist; the key is the language server capability.

Recommendation (most common choice): install **“Lua”** powered by **Lua Language Server** (the `sumneko.lua` server). More info: https://marketplace.visualstudio.com/items?itemName=sumneko.lua.

### 1b) Installing the editor (Windows / Linux / Mac)

Before you load scripts, install Visual Studio Code or VSCodium:

- **Windows**: grab the `.exe` installer from https://code.visualstudio.com or the MSI from https://vscodium.com and follow the wizard.
- **Linux**: install from `.deb`/`.rpm` downloads or via package managers (`sudo snap install code --classic`, `sudo apt install codium`, etc.).
- **Mac**: download the `.zip`/`.dmg` from each site, unzip, and move the app to `/Applications`.

VSCodium is the libre alternative (no telemetry) and provides the same extensions; just use its assets instead of Microsoft’s.
