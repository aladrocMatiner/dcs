...
Download the installer from https://code.visualstudio.com and follow the wizard for your platform:

- **Windows**: run the `.exe` and accept the defaults.
- **Linux**: install the `.deb`/`.rpm` or use `sudo snap install code --classic`.
- **Mac**: download the `.dmg`/`.zip`, unzip, and move it to `/Applications`.

### 1b) Libre alternative: VSCodium

If you prefer a telemetry-free build, VSCodium provides the same experience. Get the MSI/PKG from https://vscodium.com or install via package managers (`sudo snap install codium --classic`, `sudo apt install codium`, etc.).

### 1c) Install the Lua extension / language server

Now install a Lua extension that gives you:

- Syntax highlighting
- Diagnostics (lint-like feedback)
- Autocomplete / language server support

Look for explicit “Lua Language Server” support. The most common recommendation is **“Lua”** powered by the `sumneko.lua` server. More info: https://marketplace.visualstudio.com/items?itemName=sumneko.lua.
