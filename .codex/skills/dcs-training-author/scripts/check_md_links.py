...
Usage:
  ...
  ./check_md_links.py <root> --glob "**/*.md"
...
def strip_inline_code(line: str) -> str:
    # Remove inline code spans to avoid false positives from examples.
    out: list[str] = []
    in_code = False
    i = 0
    while i < len(line):
...
def iter_markdown_files(root: Path, pattern: str, ignores: list[str]) -> list[Path]:
...
def normalize_target(raw: str) -> str:
    # Strip surrounding whitespace and optional title after space: (path "title")
    target = raw.strip()
...
def check_file(md_path: Path) -> list[str]:
    ...
    for line_number, raw_line in enumerate(text.splitlines(), start=1):
        ...
        for match in LINK_RE.finditer(line):
            ...
            # Ignore pure anchors like (./#something) after splitting.
            if path_part in (".", "./"):
                continue

            # Resolve relative paths only (absolute paths are treated as errors by default).
...
def main() -> int:
    ...
    parser.add_argument("--glob", default="**/*.md", help="(reserved) keep for compatibility")
    ...
    md_files = iter_markdown_files(root, args.glob, args.ignore)
    all_errors: list[str] = []
